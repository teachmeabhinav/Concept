...
const STREAM_TIMEOUT_MS  = 90000;  // 90 sec to receive the FIRST token
                                   // (raised from 30s — 7B model on CPU
                                   //  can take 20-60s to generate first token
                                   //  when loading from cold start)
                                   //
                                   // NOTE: The server now sends {"status":"working"}
                                   // immediately on connection, so this timeout is
                                   // effectively disarmed within milliseconds.
                                   // It only fires if the server itself is unresponsive.
...
interface StreamChunk {
    ...
    error?:   string;
    status?:  string;   // "working" — immediate flush event sent by server at t=0
                        // used to bypass Node.js/Undici 16KB buffer threshold
    sources?: Array<{ file: string; score: number | null }>;
}
...
async function streamRAG(
    message: string,
    resetMemory: boolean,
    vsStream: vscode.ChatResponseStream,
    ...
    let buffer = '';
    let sources: Array<{ file: string; score: number | null }> = [];
...
const handler: vscode.ChatRequestHandler = async (
    ...
    try {
        ...
        if (error.name === 'AbortError') {
            stream.markdown(
                '⏱️ **No response within 90 seconds.**\n\n' +
